    ...
    return (
        <>
            <MyModal visible={modal} onClose={() => setModal(false)}>
                <p className='text-xl font-bold text-red bg-white p-6 rounded-xl'> You have to be logged in to enter your profile data. </p>
            </MyModal>
            <div style={{ display: isVisible ? 'block' : 'none' }} className={'absolute mt-7 mr-6'} onClick={onBoxClick}>
                ...
                <div className={`bg-white px-6 py-2 w-48`}>
                    <ul>
                        ...
                        <li className='message-box-link'> <a className='cursor-pointer' onClick={() => onClick('/orders')} > Sifarişlərim </a> </li>
                        <li className='message-box-link'> <a className='cursor-pointer' onClick={() => onClick('/adresses')}> Ünvanlarım </a> </li>
                        <li className={'message-box-link border-b-0'}>
                            <div className={`flex ${!isAuth ? 'flex-col' : 'flex-row'} ${!isAuth ? 'justify-center' : 'items-center'}`}>
                                ...
                                <LoginForm isVisible={loginForm} setIsVisible={setLoginForm} onLinkClick={onLinkClick} />
                            </div>
                        </li>
                    </ul>
                </div>
