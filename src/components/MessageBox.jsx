...
const MessageBox = ({ isVisible, onLinkClick }) => {
    ...
    const onClick = (link) => {
        if (isAuth) {
            ...
            setLoginForm(true);
            setLoginFormErrorMessage('Profil məlumatlarınızı əldə etmək üçün daxil olmalısınız');
        }
    }
    ...
    return (
        <>
            {isVisible &&
                <div className={'absolute mt-7 mr-6'} onClick={onBoxClick}>
                        ...
                        <ul>
                            ...
                            <li className={'message-box-link border-b-0'}>
                                <div className={`flex ${!isAuth ? 'flex-col' : 'flex-row'} ${!isAuth ? 'justify-center' : 'items-center'}`}>
                                    ...
                                    <LoginForm
                                        isVisible={loginForm}
                                        setIsVisible={setLoginForm}
                                        onLinkClick={onLinkClick}
                                        externalError={loginFormErrorMessage}
                                    />
                                </div>
                            </li>
                        </ul>
