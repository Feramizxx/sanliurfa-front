    ...
    return (
        <div className={'mr-3 mb-5 max-w-fit'}>
            ...
            <label htmlFor={props.name} className={`px-4 py-2 rounded-full text-[13px] relative group hover:bg-red hover:text-white 
                                                  ${checked ? 'bg-red text-white' : 'bg-[#F1F1F1]'}`}>
                <span className={`${props.price ? 'group-hover:invisible' : ''}`}>{props.name}</span>
                {props.price && (
                    <span className={`absolute text-white top-0 left-0 w-full h-full text-center hidden translate-y-1.5 group-hover:block`}>
                        {props.price}
                    </span>
                )}
            </label>
