...
const InlineNews = () => {
  ...
  const { id } = useParams();
  const { singleNews, singleNewsIsLoading, singleNewsError } = useFetchOneNews(id);
  const { news, newsAreLoading, newsError } = useFetchNews(value, 5);
  const navigate = useNavigate();

  if (singleNewsIsLoading || newsAreLoading) return <PageLoader />

  if (singleNewsError && singleNewsError instanceof AxiosError) {
    if (singleNewsError.response.status === 404) {
      return <Navigate to='/' />
    }
  }

  const onClickRecentNews = (id) => {
